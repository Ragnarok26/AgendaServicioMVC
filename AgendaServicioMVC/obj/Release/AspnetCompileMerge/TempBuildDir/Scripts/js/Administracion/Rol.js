    ...
    $.ajax(
        {
            ...
            success: function (data, textStatus, jqXHR) {
                if (data != null) {
                    ...
                    $("#rolChkAll").change(
                        function () {
                            var checked = $("#rolChkAll").prop("checked");
                            $(".rolChk:checkbox").each(
                                function () {
                    ...
                    $(".btnEditModal").click(
                        function (event) {
                            CargarDatosRol(event.currentTarget.value);
                        }
                    );
                    $(".btnDelete").click(
                        function (event) {
                            RolDelete(event.currentTarget.value);
                        }
                    );
                    CerrarModal();
                } else {
...
function CargarDatosRol(targetId) {
    AbrirModal(
        {
            Mensaje: "<p><h3>Cargando...</h3></p>",
            AlineacionMensaje: "center"
    ...
    $.ajax(
        {
            ...
            success: function (data, textStatus, jqXHR) {
                if (data != null) {
                    $("#rolAddEdit").html(data);
                    CargarEventosEdicion();
                    CerrarModal();
                    $("#RolAgregarEditarModal").modal("show");
                } else {
                    CerrarModal();
                    $.toast(
                        {
...
function CargarEventosEdicion() {
    $("#btnRolSave").click(
        function (event) {
            RolSave(event.currentTarget.value);
        }
    );
}

function RolSave(value) {
    AbrirModal(
        {
        ...
        $.ajax(
            {
                ...
                data: "{ rol: " + JSON.stringify(Entity) + " }",
                cache: false,
        ...
        $.ajax(
            {
                ...
                dataType: 'json',
                contentType: "application/json",
                data: "{ roles: " + JSON.stringify(Entities) + " }",
                cache: false,
                success: function (result, textStatus, jqXHR) {
                    CerrarModal();
                    if (result != null) {
