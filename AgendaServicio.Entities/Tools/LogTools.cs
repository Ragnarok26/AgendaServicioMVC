...
namespace AgendaServicio.Entities.Tools
{
    public class LogTools
    {
        ...
        private static long InsertLog(long parentId, string user, string empresa, string service, string serviceMethod, string info, DateTime date)
        {
            ...
            try
            {
                using (SqlConnection conexion = new SqlConnection(ConfigurationManager.ConnectionStrings[Pais + "." + empresa + ".LOGDB"].ConnectionString))
                {
                    conexion.Open();
                    using (SqlCommand command = new SqlCommand(insertCmd, conexion))
                    {
                        ...
                        command.Parameters.AddWithValue("@app", ConfigurationManager.AppSettings[Pais + "." + empresa + ".App.Name"]);
